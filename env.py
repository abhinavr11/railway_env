...
class GridEnv():
    ...
    def find_station_by_track_id(self,track_id):
        """
        ...
        for station_name in self.stations_list:
            start = self.station_info[station_name]["start_track"]
            cap = self.station_info[station_name]["capacity"]
    ...
    def calculate_end(self, action, start, t_start, section_length, speed):
        tau_min = 3
        if action == 'move':
    ...
    def select_action(self, train_id, curr_track, direction, t_start, epsilon, section_length, speed):
        arrival_track = -1
        station_no = self.find_station_by_track_id(curr_track)
        if np.random.random() < epsilon:
            ...
            if arrival_track == curr_track:
                action = 'halt'
                t_end = self.calculate_end('halt', curr_track, t_start, section_length, speed)
                self.grid[curr_track, t_start] = 1
                self.grid[arrival_track, t_end] = 1
            else:
                action = 'move'
                t_end = self.calculate_end('move', curr_track, t_start, section_length, speed)
            
            obs_space = self.cropping_window(curr_track, arrival_track, t_start, t_end)
        else:
            ...
            self.grid[curr_track:arrival_track+1, t_start:t_end+1] = 1
    ...
    def cropping_window(self, current_track, arrival_track, t_start, t_end):
        print("current_track", current_track, " arrival_track", arrival_track, " t_start", t_start, " t_end", t_end)
        # Calculate vertical and horizontal half-sizes
        vertical_half_size = (self.vertical_size - 1) // 2
        horizontal_half_size = (self.horizontal_size - 1) // 2

        # current_track, t_start -> oleft
        # arrival_track, t_end -> oright

        pad_width_left, pad_width_right, pad_width_up, pad_width_down = 0,0,0,0
        
        if t_start < horizontal_half_size:
        ...
        print("pad_width_left, pad_width_right, pad_width_up, pad_width_down", pad_width_left, pad_width_right, pad_width_up, pad_width_down)

        global global_grid 
        global padded_gr
        padded_grid = np.pad(copy.deepcopy(self.grid), 
                             pad_width=((pad_width_up, pad_width_down), 
        ...
        print("padded_grid", padded_grid)
        print("o_left", o_left)
        print("o_left.shape", o_left.shape)

        print("Self.grid.shape", self.grid.shape)
        print("=----------------------------------------------------=")
        # Calculate the slice indices for o_right (arrival_track and t_end)
        print("t_enc", t_end)
        print("arrival_Track:", arrival_track)
        pad_width_left_r, pad_width_right_r, pad_width_up_r, pad_width_down_r = 0,0,0,0
        print("t_end", t_end, "horizontal_half_size", horizontal_half_size)
        print("self.grid.shape[1]", self.grid.shape[1])
        print("arrival_track", arrival_track)
        print("grid.shape[0]", self.grid.shape[0])

        if t_end < horizontal_half_size:
        ...
        vertical_start_right = arrival_track +pad_width_up_r- vertical_half_size
        vertical_end_right = arrival_track + pad_width_down_r - self.grid.shape[0]
        horizontal_start_right = t_end - horizontal_half_size + pad_width_left_r
        horizontal_end_right = t_end + horizontal_half_size - self.grid.shape[1]
        print("vertical_start_right, vertical_end_right, horizontal_start_right, horizontal_end_right", vertical_start_right, vertical_end_right, horizontal_start_right, horizontal_end_right)

        print("self.grid.shape", self.grid.shape)
        padded_grid_right = np.pad(copy.deepcopy(self.grid), 
                             pad_width=((pad_width_up_r, pad_width_down_r), 
                                        (pad_width_left_r, pad_width_right_r)), 
                             mode='constant', constant_values=0)
        
        print("padded_grid_right:", padded_grid_right.shape)
        global_grid = self.grid
        padded_gr = padded_grid_right

        # Extract the right window from the padded grid

        
        
        o_right = padded_grid_right[
              : self.vertical_size,
             : self.horizontal_size]        
        print("oright", o_right)
        print("oright.shape", o_right.shape)
        return [o_left, o_right]

    
    def step(self, ostep, action, arrival_track, train_id, epsilon, section_length, speed ):
        ...
        station_no = self.find_station_by_track_id(start)
        ...
        if action == 'move':
            ...
            station_no_step_1= self.find_station_by_track_id(arrival_track)
            _, _, ostep_1 = self.select_action(train_id, arrival_track, direction, t_end, epsilon, section_length, speed) # curr_track, station_no, direction, t_start
            ...
            if self.grid[start,t_start-1] > 1: #  REVIEW THIS !!!!!!!!!!!  If previous time_step is not occupied by train(that is 1), do this
                ...
                self.train_state[train_id][1] = t_end =  t_start+tau_min
                station_no_step_1= self.find_station_by_track_id(arrival_track)
                
                _, _, ostep_1 = self.select_action(train_id, arrival_track, direction, t_end, epsilon, section_length, speed)           
                return  ostep, action, ostep_1, reward, done
            # If next section is free and it is not the first time halting do one step
                ...
                self.train_state[train_id][1] = t_end = t_start+1
                station_no_step_1= self.find_station_by_track_id(arrival_track)
                _, _, ostep_1 = self.select_action(train_id, arrival_track, direction, t_end, epsilon, section_length, speed)
                return  ostep, action, ostep_1, reward, done
            # If next section is not free find the timestep corresponding to when next section is available.
                ...
                self.train_state[train_id][1] = t_end = non_zero_indices+1
                station_no_step_1= self.find_station_by_track_id(arrival_track)
                _, _, ostep_1 = self.select_action(train_id, arrival_track, station_no_step_1, direction, t_end, epsilon, section_length, speed)
                return  ostep, action, ostep_1, reward, done
...
if __name__ == "__main__":
    ...
    # a = env.step('move','arrival_track', 'train_id', 'epsilon' 0) #action, start, connecting_edge, end, t_start, train_id, destination
